package com.allen.component.redis.repository;
...
/**
 * @author Allen
 * @date 2022/10/18 14:00   存储存字符传的redis工具类
 **/
@Repository
@DependsOn
public class RedisRepository {
	...
	public RedisRepository(RedisTemplate<String, Object> redisTemplate, RedisConnectionFactory redisConnectionFactory) {
		this.redisTemplate = redisTemplate;
		this.redisConnectionFactory = redisConnectionFactory;
	}

	private RedisTemplate<String, Object> currentRestTemplate() {
		return this.redisTemplate;
	}

	private SetOperations<String, Object> getSetOpr() {
	...
	public Boolean setExpire(String key, long expire) {
		LOGGER.info("setExpire key:{},expire:{}",key,expire);
	...
	public void setExpire(final String key, final String value, final long time) {
		redisTemplate.execute((RedisCallback<Long>) connection -> {
			...
			LOGGER.info("[redisTemplate redis] setExpire 放入 缓存  url:{} ========缓存时间为{}秒", key, time);
			return 1L;
		});
	}
	...
	public boolean setNxex(String key,long time) {
		try {
			// setIfAbsent 原子性
			Boolean result = redisTemplate.opsForValue().setIfAbsent(key, String.valueOf(1), time, TimeUnit.SECONDS);
			if (result != null) {
				return result;
			}
		} catch (Throwable e) {
	...
	public Long setAdd(String key, String... values) {
		LOGGER.info("setAdd key:{},values:{}",key,values);
	...
	public Set<Object> setMembers(String key) {
		LOGGER.info("[redisTemplate redis]放入 缓存  key:{}", key);
	...
	public Map<String, String> getKeysValues(final String keyPatten) {
		...
		return redisTemplate.execute((RedisCallback<Map<String, String>>) connection -> {
			...
			Map<String, String> maps = new HashMap<>(keys.size());
			for (String key : keys) {
