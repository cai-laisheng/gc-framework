...
![LockDemo debug下 .png](aqs/LockDemo%20debug下.png)
...
![自旋获取锁整体示意图.png](aqs/自旋获取锁整体示意图.png)
...
![当前节点引用线程获取锁，当前节点设置为队列头结点.png](aqs/当前节点引用线程获取锁，当前节点设置为队列头结点.png)
...
![独占式锁获取（acquire()方法）流程图.png](aqs/独占式锁获取（acquire()方法）流程图.png)
...
![超时等待式获取锁（doAcquireNanos()方法）](aqs/超时等待式获取锁（doAcquireNanos()方法）.png)
