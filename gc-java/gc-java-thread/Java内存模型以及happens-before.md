...
![JMM内存模型的抽象结构示意图](happens-before/JMM内存结构抽象结构示意图.png)
...
![从源码到最终执行的指令序列的示意图](happens-before/从源码到最终执行的指令序列的示意图.png)
...
![JMM层级图](happens-before/JMM层级图.png)
...
![JMM设计示意图](happens-before/JMM设计示意图.png)
从图可以看出：
...
![happens-before与JMM的关系](happens-before/happens-before与JMM的关系.png)
